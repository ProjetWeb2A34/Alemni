...
class paiementCrud {
    ...
    function Ajouterpaiement($paiement) {
        ...
        if ($checkStmt->rowCount() == 0) {
            die('Error: No matching id_p found in panier for the provided id_produit.');
        }
...
function Modifierpaiement($paiement, $idpai) {
        try {
            $db = config::getConnexion();
            $query = $db->prepare("UPDATE paiement SET num_cart=:num_cart, mot_cart=:mot_cart, email=:email, prix_p=:prix_p WHERE idpai=:idpai");
            $query->execute([
                'num_cart' => $paiement->get_num_cart(),
                'mot_cart' => $paiement->get_mot_cart(),
                'email' => $paiement->get_email(),
