...
if(isset($_POST['signup'])){
    $name = htmlspecialchars($_POST['name']); // Escape user input
    ...
    if(count($errors) === 0){
        ...
        $status = "notverified";
        $insert_data_query = "INSERT INTO user (id, name, phone, email, password, role)
                             VALUES (null, :name, :phone, :email, :password, :role)";
        $stmt = $db->prepare($insert_data_query);
        $stmt->execute(array(
            ':name' => $name,
            ':phone' => null, // Assuming phone is not provided in the form
            ':email' => $email,
            ':password' => $encpass,
            ':role' => null // Assuming role is not provided in the form
        ));
        if($stmt){
            $subject = "Email Verification Code";
            $message = "Your verification code is $code";
            $sender = "From: safahajji9@gmail.com";
...
if(isset($_POST['check-email'])){
    $email = htmlspecialchars($db->quote($_POST['email']));
    $check_email = "SELECT * FROM user WHERE email='$email'";
